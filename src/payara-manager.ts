...
import { waitFor } from './utils/polling.js';
...
export class PayaraManager {
   ...
   */
  private async writeSetenvConfInternal(): Promise<void> {
  ...
  private async waitForHealthy(timeoutMs: number): Promise<void> {
    await waitFor(
      ...
      {
        intervalMs: 2000,
        timeoutMessage: `Payara did not become healthy within ${timeoutMs}ms`,
      }
    );
  }
